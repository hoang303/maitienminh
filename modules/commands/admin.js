...
`, event.threadID, event.messageID);
    ...
    switch (args[0]) {
        ...
        case "add": { 
            if (event.senderID != 100048121433546) return api.sendMessage(`𝐐𝐮𝐲𝐞̂̀𝐧 của bạn không đủ ! 🎀`, event.threadID, event.messageID)
        ...
        case 'ibonly': {
        if (permssion != 3) return api.sendMessage("𝐐𝐮𝐲𝐞̂̀𝐧 của bạn không đủ ! 🎀", threadID, messageID);
    ...
    case 'only':
        case '-o': {
            //---> CODE ADMIN ONLY<---//
            if (permssion != 3) return api.sendMessage("𝐐𝐮𝐲𝐞̂̀𝐧 của bạn không đủ để thực hiện ! 🎀", threadID, messageID);
